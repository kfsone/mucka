...
///
/// <para><b>Kept deliberately.</b> Existing session recordings and several tests read exactly this
/// shape - see mudsharp.Tests' wyvern-poison-death fixture. It is also the format a plain-text
/// capture for other players would want. It is additive-only alongside <see cref="SqliteWireLogSink"/>;
/// retiring it is a separate decision.</para>
///
/// <para><b>What it costs, so the choice is informed.</b> Measured on 40 captures (MB =
...
/// six-character <c>0xFF</c> escape. That is 1.924 MB per play-hour against the SQLite sink's 0.76 MB -
/// 2.5x the disk for the same traffic.</para>
///
...
/// string, so the on-disk bytes match exactly.</para>
///
