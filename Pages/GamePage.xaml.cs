...
public partial class GamePage : ContentPage
{
    ...
    protected override void OnAppearing()
    {
        ...
        if (_antiIdleTimer == null)
        {
            ...
            try
            {
                ...
                {
                    // No window root means Invariant #0 has NO enforcement this session. That is
        ...
        // Escape closes the About overlay if it is open, then the $SCORE panel; otherwise it clears
        // the input. The TwoWay binding pushes the empty string to the Entry (no native-textbox sync
        // needed as on Windows).
        _androidEscapeHandler = () =>
        {
            if (!TryCloseAbout() && !_vm.ScoreGraph.TryClose())
    ...
    private void FocusInput() => Dispatcher.Dispatch(() =>
    {
        ...
        FocusDiag($"FocusInput dispatch: focused={hasFocus} (maui.IsFocused={InputEntry.IsFocused})");
        if (!hasFocus)
    ...
    private void OnInputHandlerChanged(object? sender, EventArgs e)
    {
        ...
        if (InputEntry.Handler?.PlatformView is Microsoft.UI.Xaml.Controls.TextBox tb)
        {
            ...
            // The command box now exists (or is a new platform view). Invariant #0's guard marks
            // nothing until it knows which control must keep the keyboard, so this is what arms it.
            _focusGuard?.Refresh();
#if INPUT_DIAG
    ...
    private void RegisterCommandInputBindings(Mucka.Input.CommandInput input)
    {
        ...
        // Escape closes the About overlay if it is open, then the $SCORE panel, else empties the box.
