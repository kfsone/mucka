...
/// lookup (see <c>MudSharp.Combat.HistoryIndex</c>).
///
...
/// rows, making the encounter's "now" and its "usual" baseline identical at n=1. Keying purely on
/// (instance, weapon, encounter start) instead means the cache is filled AT MOST ONCE per encounter,
/// strictly BEFORE that encounter's own flush can happen (Resolve is only ever called while
/// <c>_hasCombatData</c> is true, which starts becoming true on the FIRST combat event, long before
/// any close), and is never invalidated by anything except a genuinely different
/// encounter/target/weapon. It therefore cannot observe its own just-added rows - self-comparison is
/// impossible by construction, not filtered out, exactly as <c>HistoryIndex</c>'s own class remarks
/// describe for the index side of this same guarantee.</para>
/// </summary>
