...
Cases 3, 5 and 8 rest on a verbatim capture, shown below. The rest are attested by the code and
sentence-wording frequency table in "What the capture settled about the codes".
...
**One member observed, more expected.** Poison is the observed one, below. The outcome is named for
what the player lost rather than for how it happened: the regex matches `The X drops dead,
<cause>...` for any cause rather than the word "poisoned", and a genuinely new wording maps to this
same outcome as a new `CombatEventKind`. That split is the point of having an event kind (an
observed line) and an outcome (what the fight was worth) rather than one thing.

### The observed member: poison

Found by the owner noticing the client still claimed he was fighting a wyvern that was lying dead
in front of him.

**Two occurrences, and they are not the same fight.** Keeping them apart matters: A is a
recollection, B is bytes.
...
`+26 = 5,201` is not 239 either. Two observations; not a mechanism. But it is a second reason not to
call this a kill.
...
- and a line naming a creature **we are not fighting** is ignored. MUD2 stacks several end messages
  in a frame and one can land after another has already closed the fight -- the wyvern frame does
  exactly that, the poison death closing it two lines before the `08.12` arrives.
...
Every wording bug this file records -- a creature's failed flee, the player's failed flee, a named
`no longer` line -- was **correctly coded on the wire and missed anyway**, because the tracker read
only the prose. The code was right all three times.

But the division of labour is sharper than "code good, prose bad", measured across all 31 captures:
...
**A failed flee carries the same `08.11` as a real one, 19 to 0.** Bartle's wording explains why --
...
The reason `CombatTracker.NoteRoomChanged` exists: **movement is refused while fighting** -- leaving
costs a flee, which prints its own line. So a room change is proof the fight is over, independent of
having matched any sentence at all. `MudSession` calls it whenever the room short description
changes (changes, not merely arrives: `look` reprints the same room and looking around mid-fight is
free).
...
Three of the eight ends were found by a player noticing the readout was wrong -- cases 3 and 5 in one
capture, case 8 in a later one -- each in a frame that a careful reading of the then-current list said
...
Corollary from `MUD2-mechanics-verification.md`, unaffected and still interesting: each failed
...
written at all. Re-reducing the original captures is the only honest way to recover them.
...
(`MudSession.SetupCommands`); previously neither was, which is why the two sessions that prompted
this document recorded almost no swings.

Both are **sets, not toggles**, so the batch is safe to fire unconditionally on every game-mode
entry -- no "is it already on?" probe needed, and no risk of turning off a setting a persona
already had.
