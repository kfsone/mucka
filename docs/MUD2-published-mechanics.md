...
Transcribed from TheMudWiz's MUD2 Strategy Guide (v0.42, updated 2024-03-27) on GameFAQs - the
`Combat and Stats` and `Mobiles` sections.
...
>
> Its current findings are in `MUD2-mechanics-verification.md`. Anything this client SHOWS the
> player must rest on what we have verified; anything from this document alone belongs in
> analysis and design discussion, not on the combat rail. That ordering is not pedantry - this is
> a permadeath game, and a confident readout built on an unchecked player FAQ is exactly the kind
> of thing that gets a character killed while its owner trusts the panel.

Companion data: `bestiary.tsv` (alongside this document) - all 143 rows of the guide's mobile
table, tab-separated.
...
captures (`MUD2-mechanics-verification.md`). This is the mechanic behind the owner's repeated
deaths to rats - at low stamina your dexterity is falling, which raises *their* hit chance,
which is why three rats can all land on one tick instead of the usual ~0.7 of them.

**There is a third knee at 20, and it is not a stat formula.** It differs from the stat-formula
knees in kind, not merely in degree: it is not something the engine computes, but something the
player must decide about. Three thresholds, three different kinds of thing:
...
Outside rats, of 5 occasions at exactly 20 stamina, 3 cost the character. That is the number that
matters. It is a small sample and it is also lived experience in a permadeath game, and it
outranks any formula here for the purpose of deciding what the panel shouts about.
...
The opportunistic-join warning ("being visibly wounded attracts new attackers") is not a
heuristic - it is this function crossing a published threshold.
...
### A more precise model, from play, which does not agree in shape
...
- At about **7 stamina** the loss is small - roughly 500-600 points on a ~46,000 score, so
  roughly 1.2%.
...
The guide's bands are **percentages of maximum stamina**; the play-derived thresholds are
**absolute**. On this character they nearly coincide - max stamina 105, so the guide's 11-25%
band spans 12-26 and its free band ends at 10, against the absolute figures of 20 and 6 - which is
presumably why a FAQ author with a ~100-stamina character wrote it in percentages at all.

**They diverge sharply for anyone else.** A low-level character with 30 maximum stamina would, on
the guide's reading, flee free below 3 and pay half up to 7; on the absolute reading they flee free
...
flee cost reads as INSUFFICIENT, which is honest, and the absolute figures are the ones to trust
for this character because they come from play rather than from a transcription.

**Does the threshold scale above 100 maximum stamina?** Maximum stamina is permanently boostable
to 120, and it has never been tested whether going above 100 changes anything - a threshold that
is "20" on a 100-stamina character is exactly 20%. This character sits at 105, so it cannot
distinguish the two readings either. Untested, and worth testing before anyone hard-codes 20.
...
full-health flee, at the published `400%`, cost **~36% of total score**. The play-derived figure is
...
quantity being described from play. Our measurement fits the play-derived model - flat 10% to 20,
steep drop below - and does not fit the guide's structure at all.
...
**They are not destroyed - they are paid to the attackers you fled from.** Fleeing the ram gave
the ram points and potentially levelled it, and killing it afterwards in revenge returned only a
fraction: 313 recovered against 2,079 lost.

This resolves an anomaly the mechanics-verification pass flagged and could not explain. The ram
kill awarded **313 points against a published `Points` value of 106**, while every other species
matched its table entry exactly. The ram had been *promoted by the owner's own flee* moments
earlier. So the bestiary's `Points` is the value of a creature at its base rank, and a creature
that has eaten a player's flight is worth more than the table says.

**Creature value and rank are directly queryable** with the `value` command, per NPC. That is the
route to grounding this - and to explaining the +4 seen on each of the water-snake's failed
escapes, which the same mechanism likely runs the other way.
...
**This table stays out of the UI** - the numbers, not the subject. The flee pill
(`combat-rail-spec.md`, section 5) prices a flee by the formula in `MUD2-flee-cost.md`, not by
the percentages guessed here.
