...
/// Derived from Clio telnet.l lines 305–354 SGR switch statement.
/// </summary>
public class AnsiSgrTests
{
    ...
    {
        // SGR 31 — Clio: wfore(mwin, 31-30) = wfore(mwin, RED=1)
        var h = new ParserHarness();
        h.Feed("\x1B[31m");
        h.Feed("text\n");
        Assert.Single(h.Lines);
    ...
    {
        // SGR 96 — bright foreground: 96-90+8 = 14 = BrightCyan
        var h = new ParserHarness();
        h.Feed("\x1B[96m");
        h.Feed("text\n");
        Assert.Single(h.Lines);
    ...
    {
        // SGR 43 — Clio: wback(mwin, 43-40) = wback(mwin, YELLOW=3)
        var h = new ParserHarness();
        h.Feed("\x1B[43m");
        h.Feed("text\n");
        Assert.Single(h.Lines);
    ...
    {
        // ESC[1;31m — Clio: ansi_bold=1 then wfore(mwin, 31-22)=wfore(mwin, RED=9=BrightRed)
        // MudSharp applies bold flag + Red fg independently (Bold=true, Foreground=Red)
        var h = new ParserHarness();
        h.Feed("\x1B[1;31m");
        h.Feed("text\n");
        Assert.Single(h.Lines);
