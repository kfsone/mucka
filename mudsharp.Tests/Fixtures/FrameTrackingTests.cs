...
public class FrameTrackingTests
{
    ...
    private static ParserHarness InGameMode()
    {
        ...
        h.Feed(0x9D, 0x9C, 0xFF, 0xFF);  // Enter game mode (sets LT_GREEN c)
        h.Feed(0x9B, 0xFF, 0xFF);          // C00: init_stack → reset c to WHITE/BLACK
        h.Lines.Clear();
        return h;
    }
    ...
    public void RoomShortReady_FiredAfterCommandEchoClearsFrameStart()
    {
        ...
        // c arrives — c-based detection must still fire regardless.
        var h = InGameMode();
        h.Feed(WithPrompt("You go north.\n"));
