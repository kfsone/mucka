...
public class GamePromptTests
{
    ...
    [Fact]
    public void WirePrompt_AfterPrompt_NextTextIsNotPromptColor()
    {
        // After the wire prompt's two C1 pops complete, subsequent text must not inherit
        // the prompt's BLUE or LT_BLUE color.  The PopColor() stack unwind must restore
        // either the pre-prompt color (if any) or TextStyle.Default (if the stack is empty),
        // never stale prompt blue.
        var h = InGameMode();   // pushes LT_GREEN (BrightGreen) as base game-mode color
        h.Feed(WithPrompt("You arrive in the tearoom.\n"));
        h.Lines.Clear();
        h.Feed("go north\r\n");
