...
public class SoundTriggerTests
{
    ...
    {
        // Backs the ctrl-r reply hotkey: a named sender must be reported so the client can
        // track "last person to tell me something" independent of the game's own 're' command,
        // which re-resolves its target at send time.
        var h = InGameMode();
        h.Feed(0xA4, 0x9E, 0xFF, 0xFF);
        h.Feed("Ollie tells you \"hello\".\n");
