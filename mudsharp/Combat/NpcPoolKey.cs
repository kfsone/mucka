...
/// dagger is concerned) and the wrong one for a pool: measured against the corpus, a "large rat0"
/// runs to roughly 100 stamina where a plain "rat0" runs to roughly 25. Pooling those under one name
/// produces a band four times too wide, and one that is wrong at both ends.</para>
///
/// <para><b>Why the instance number and nothing else is stripped.</b> Per-instance behaviour
/// modelling is measured to be wasted effort - rat3 and rat7 are statistically indistinguishable
/// (17 instances, p=0.80) and per-instance pool estimates agree within a species - so the number
/// carries no information about the pool and dropping it is what buys the sample size. Everything
