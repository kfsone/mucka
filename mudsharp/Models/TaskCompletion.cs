...
/// time; the running count is the number of the eight now done. One capture, a dragon: the line is
/// followed by <c>(Persona saved on +100 = 5,144).</c> and then <c>(Persona saved on +976 = 6,120).</c></item>
/// <item><c>You have completed a Task which you have done before.</c> - no count, and it still pays.
/// A second capture: <c>+100 = 16,739</c> then <c>+976 = 17,715</c>.</item>
/// </list>
/// A third capture, a water-snake, has the same shape: task line, then <c>+100</c>, then <c>+84</c>.
/// In all three the task payout is printed FIRST and the combat award second, which is the ordering
/// everything downstream relies on.</para>
///
/// <para><b>The line carries no C1 code.</b> Verified against the raw bytes of a captured
