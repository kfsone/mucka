...
/// <summary>
/// Tunables for <see cref="ResetClock"/>. Defaults target a 5 s FES heartbeat and the "lock once,
...
internal sealed class ResetClockOptions
{
    ...
    /// <summary>Spacing between discovery samples. Kept just above ~500 ms so each FES probe is
    /// answered with a stats block (not a bare prompt). One in flight at a time, so the effective
    /// cadence is max(this, RTT); a 60 s edge brackets to ~ this/2.</summary>
    ...
    /// collision / asleep). Covers a slow RTT.</summary>
