...
WITH fight_rollup AS (
    SELECT
        ...
        COUNT(CASE WHEN outcome = 'Kill' THEN 1 END) AS kills,
        COUNT(CASE WHEN outcome = 'Unresolved' THEN 1 END) AS passes,
        COUNT(CASE WHEN outcome = 'CFled' THEN 1 END) AS npc_flees,
        COUNT(CASE WHEN outcome = 'UFled' THEN 1 END) AS your_flees,
        COUNT(CASE WHEN outcome = 'Withdraw' THEN 1 END) AS withdrawn,
        COUNT(CASE WHEN outcome = 'CFledFail' THEN 1 END) AS npc_flee_fails,
        COUNT(CASE WHEN outcome = 'UFledFail' THEN 1 END) AS your_flee_fails,
...
SELECT
    ...
    COUNT(CASE WHEN outcome = 'Kill' THEN 1 END) AS kills,
    COUNT(CASE WHEN outcome = 'Died' THEN 1 END) AS deaths,
    COUNT(CASE WHEN outcome = 'CFled' THEN 1 END) AS npc_flees,
    COUNT(CASE WHEN outcome = 'UFled' THEN 1 END) AS your_flees,
    COUNT(CASE WHEN outcome = 'Withdraw' THEN 1 END) AS withdrawn,
    COUNT(CASE WHEN outcome = 'CFledFail' THEN 1 END) AS npc_flee_fails,
    ...
    ROUND(AVG(CASE WHEN outcome = 'Kill' AND narrative_mode = 0 THEN approx_damage_done END), 3)
        AS est_stamina_pool,
